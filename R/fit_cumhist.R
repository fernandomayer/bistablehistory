...
fit_cumhist <- function(data,
                        state,
                        duration=NULL, onset=NULL,
                        random_effect=NULL, session=NULL, run=NULL,
                        fixed_effects=NULL,
                        norm_tau=c(5, 0.2),
                        mixed_state=0.5,
                        history_mix=0.0,
  ...
  if (is.null(duration)) {
    # we must infer duration of individual phases from onsets
  ...
  # random effects
  if (max(cumhist$data$random_effect) > 1) {
    cumhist$data$random_levelsN <- max(cumhist$data$random_effect)
    cumhist$data$clear_random_effect <- cumhist$data$random_effect[cumhist$data$is_used == 1]
